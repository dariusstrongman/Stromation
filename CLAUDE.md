...
- [ ] The Brass Effect Division 27 Auto-Bidder -- IN PROGRESS
  ...
  - [x] Pipeline v16-arch (2026-04-15) — ROOT-CAUSE FIXES (not patches):
    - **Real root cause #1 found**: `run_pass2` was SUMMING device counts across ALL drawing pages. WF3's prompt has been returning `floor_identifier` + `sheet_series` per page for cross-page dedup, but the aggregator ignored them. Same data outlets shown on T1.0 (technology) + ES1.0 (security) + E2.0 (power) → counted 3x. This is why Dunkin' showed 29 drops vs ~13 real, Texas Roadhouse 54 vs ~30-40 real.
    - **Fix**: `extract_counts_from_result` now returns `(counts, meta)` tuple including floor_identifier + sheet_series. `run_pass2` buckets drawing counts by (normalized_floor, device_key), takes MAX within bucket (cross-series dedup on same floor), then SUMS across distinct floors. Floor names normalized: "1st floor", "1st flr", "level 1", "1" all → "F1". Roof/basement/mezzanine handled separately. Pages without floor info bucketed by sheet_series prefix as fallback.
    - **Validation**: simulation with T+ES+E pages all showing same 1st-floor 54 drops + T2.0 with 40 drops + roof page with 2 drops:
      - OLD logic: 244 drops, 28 cameras (3x over-count)
      - NEW logic: 96 drops, 14 cameras (correct: 54+40+2 = 96, 8+6 = 14)
  ...
  - [x] Div 27 strict scope change (2026-04-16, evening):
    - **Per Darius**: Antonio only does Division 27. No Division 28 (access control, intrusion). Cameras = install only (hang + cable, owner furnishes hardware/NVR). No POS terminals.
    - **WF4**: Removed camera hardware (Hikvision/Avigilon/i-Pro units, NVR, licenses, mounts). Removed access control handler (card readers, strikes, COSEC/HID panels). Removed intrusion handler (Bosch panels, motion, keypads). Removed POS terminal auto-add. Camera labor reduced 4→2.5 hrs (no programming). Programming hours cut (no AC/intrusion/NVR config).
    - **Pipeline**: DEVICE_TO_SYSTEM mapping drops access_control_doors, motion_detectors, keypads, glass_break_sensors, door_contacts. SECURITY_KEYWORDS stripped to cameras only. ACCESS_CONTROL_KEYWORDS emptied. POSITIVE_KEYWORDS: removed "access control", "intrusion", "card reader", "division 28". valid_device_keys drops all AC/intrusion. Crosscheck prompt drops access_control_doors. Legend hints drop card reader. Spec extraction drops access_control_spec.
    - **WF3**: count_devices JSON response template stripped to `security: {fixed_cameras, ptz_cameras}` only. Explicit "DO NOT COUNT" for access control + intrusion in prompt.
    - **WF6**: Removed access_control_doors, motion_detectors, keypads from flat() function. scopeGuard updated: "cameras (install only)" instead of "security (cameras, intrusion)". Removed access control guard entirely.
    - **Cron**: gap_reconcile removed AC/intrusion from securityTotal calc + acDoors check. auto_promote sets acDoors=0.
    - **Dashboard**: Removed Access Control badge from scope chips. Device names/categories stripped of AC/intrusion (access_control_doors, motion_detectors, keypads, glass_break_sensors, door_contacts). Generate Quote form no longer sends these fields. Camera label changed to "Cameras (install only)".
    - **Data cleanup**: 4 bids had stale Div 28 devices in device_analysis (JP Starks 11 AC doors + 5 door contacts, Dunkin' 2 AC doors, 7Brew-Waco 1 keypad + 1 AC door, Talkin' Taco 1 AC door). All stripped + re-quoted. 7Brew-Waco $54K→$35K, Talkin' Taco $39K→$11K.
    - **Remaining API key refs**: All 7 workflow files have hardcoded Supabase/Anthropic/Gemini keys inline. Should move to n8n credentials or env vars. Not blocking but flagged.
  - [x] WF4 pricing overhaul — calibrated to Antonio's real winning bids (2026-04-18):
    - **Problem**: system was 61% over Antonio's real $5,656 Asian Beauty Products bid ($9,100 vs $5,656). Root cause: Panduit Cat6A default ($1,414/box) when Antonio uses Wavenet Cat5E ($246/box), UPS/PDU/switch auto-added ($1,085) when Antonio says "network equipment by others", 12U rack ($520) when 6U ($293) is right, $1,200 freight when $130 is real.
    - **Wavenet vendor profile added**: full parts table from Antonio's real bid — WV-5E04URBL4 Cat5E cable ($246), WV-HD5EKSJWH jacks ($1.39), WV-FP02PWH faceplates ($0.85), WV-PC05-5E patch cords ($1.92), WV-6EPP48/24 patch panels ($134/$79), WV-CBTF-WM6U 6U rack ($293), WV-J-HOOK ($6), threaded rods ($12), firestop ($129+$67), grounding ($120), misc ($180), labeling ($60).
    - **Default vendor = Wavenet for QSR/commercial**: `resolveVendors()` no longer defaults to 'panduit'. Returns empty string, tier-based selection picks wavenet for QSR/commercial, panduit only for enterprise or when spec explicitly requires it.
    - **UPS, PDU, switch removed from default scope**: Antonio qualification #10: "Pricing does not include UPS and PDUs or any scope related to networking equipment." Only added when spec explicitly calls for them via `tryAddFromSpec()`. Enterprise tier still gets them.
    - **Rack sizing**: 6U Wavenet for QSR (was 12U Middle Atlantic). J-hooks + threaded rods + firestop now itemized individually (matching Antonio's real line items).
    - **Freight**: $130 QSR (was $1,200), $500 commercial, $1,200 enterprise only.
    - **PM hours**: 4 for small jobs (≤40 field hrs), 8 for medium, scaled 18% for large. Was 22% of field hours for all.
    - **Labor formula**: QSR rack hours = 0 (built into per-drop rate, matching Antonio's 32 hrs for 16 drops + 1 rack). No 15% commissioning for QSR. Commercial/enterprise keep rack hours (14/20) + 10% commissioning.
    - **A/B result**: system now quotes $4,966 vs Antonio's $5,656 = **-12.2%** (was +61%). Labor matches exactly ($3,270). Materials slightly lower because system optimizes cable box count (1 vs Antonio's 3) and uses 24-port panel (vs Antonio's 48-port). Every infrastructure line item (rack, firestop, grounding, misc, labeling, supplies) matches to the penny.
    - **Remaining delta explained**: Antonio over-provisions cable boxes (3 vs calculated 1) for waste/spares margin, and uses 48-port panel for future-proofing. These are legitimate estimator judgments, not system errors.
  - [ ] Gemini free tier RPD cap (20/day) needs billing enabled — click "Set up billing" at aistudio.google.com/rate-limit. Free $300 credit, no actual charges at our volume. Removes the 20 RPD cap that blocks Gemini verify.
